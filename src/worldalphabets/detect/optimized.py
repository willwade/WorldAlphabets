...
from . import (
    PRIOR_WEIGHT, FREQ_WEIGHT, CHAR_WEIGHT, DEFAULT_FREQ_DIR,
    _tokenize_words, _tokenize_bigrams, _tokenize_characters,
    _load_rank_data, _overlap, _character_overlap, _frequency_overlap
)
from ..helpers import get_language

# Performance constants
HIGH_CONFIDENCE_THRESHOLD = 0.8
COMMON_LANGUAGES = ['en', 'es', 'fr', 'de', 'it', 'pt', 'ru', 'zh', 'ja', 'ar', 'hi', 'ko']
...
class ProgressCallback:
    ...
    def update(self, status: str, processed: int, total: int):
...
def _load_char_index() -> Optional[Dict]:
    ...
    try:
        char_index_path = Path(__file__).parent.parent.parent.parent / 'data' / 'char_index.json'
...
def _load_script_index() -> Optional[Dict]:
    ...
    try:
        script_index_path = Path(__file__).parent.parent.parent.parent / 'data' / 'script_index.json'
...
def get_candidate_languages_from_text(text: str, all_languages: List[str]) -> List[str]:
    ...
    prioritized = [
        lang for lang in candidates if lang in COMMON_LANGUAGES
    ] + [
...
def optimized_detect_languages(
    ...
    Args:
        ...
        candidate_langs: List of candidate languages (if None, uses character-based filtering)
    ...
    if candidate_langs is None:
        try:
            ...
            progress.update("Filtered candidates using character analysis", 0, len(candidate_langs))
        except Exception:
    ...
    for i, lang in enumerate(prioritized_langs):
        if found_high_confidence and enable_early_termination:
            progress.update("Early termination - high confidence match found", len(prioritized_langs), len(prioritized_langs))
            break
        ...
        try:
            ...
            if word_score > 0.05:
                ...
                if enable_early_termination and word_score > HIGH_CONFIDENCE_THRESHOLD:
                    ...
                    progress.update(f"High confidence match found: {lang}", i + 1, len(prioritized_langs))
            ...
            if use_character_fallback and text_chars:
                ...
                if alphabet_data:
                    ...
                    freq_overlap_score = _frequency_overlap(text_chars, char_frequencies)
                    ...
                    final_char_score = PRIOR_WEIGHT * priors.get(lang, 0.0) + CHAR_WEIGHT * char_score
    ...
    progress.update("Finalizing results", len(prioritized_langs), len(prioritized_langs))
