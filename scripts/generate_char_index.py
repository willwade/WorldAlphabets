...
def generate_character_index(alphabet_data: Dict[str, Dict]) -> Dict:
    ...
    char_to_languages_json = {
        char: list(languages) 
        for char, languages in char_to_languages.items()
    }
...
def generate_script_index(alphabet_data: Dict[str, Dict]) -> Dict:
    ...
    script_to_languages_json = {
        ...
        for script, languages in script_to_languages.items()
    }
...
def main():
    ...
    print(f"   - {char_index['metadata']['languages_with_frequencies']} with frequencies")
    ...
    char_counts = {
        char: len(langs) 
        for char, langs in char_index['char_to_languages'].items()
    }
    common_chars = sorted(char_counts.items(), key=lambda x: x[1], reverse=True)[:10]
    print(f"   Most common chars: {[f'{char}({count})' for char, count in common_chars]}")
