...
def sync_alphabets(source_dir: Path, target_dir: Path) -> int:
    ...
    print(f"Copied {copied} alphabet files from {source_alphabets} to {target_alphabets}")
    return copied
...
def sync_frequency_data(source_dir: Path, target_dir: Path) -> int:
    ...
    print(f"Copied frequency data directory with {freq_files} files from {source_freq} to {target_freq}")
    return freq_files
...
def main():
    ...
    if not source_dir.exists():
        ...
        print("Please run the data pipeline first: uv run scripts/build_data_pipeline.py")
        sys.exit(1)
    ...
    print()
    print(f"✅ Sync complete! {total_files} files synced to Python package data directory")
    print()
    print("Next steps:")
    print("1. Build the package: uv run python -m build")
    print("2. Test the package: uv run python -c 'from worldalphabets import get_available_codes; print(len(get_available_codes()))'")
