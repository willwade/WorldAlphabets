...
import { INFLECTIONS, INFLECTION_INDEX, TAG_MAP } from './dist/browser-inflections.mjs';
...
function joinWordsSync(rulesData, prev, nextWord) {
...
export async function joinWords(locale, prev, nextWord) {
  const rulesData = await loadInflectionRules(locale);
  return joinWordsSync(rulesData, prev, nextWord);
}
...
export async function getFeatures(tag) {
...
export async function applyRules(locale, text) {
  ...
  while (i < tokens.length) {
    if (results.length > 0) {
      ...
      const jr = joinWordsSync(rulesData, prevToken, tokens[i]);
  ...
  _applyJoins(tokens, joinRules) {
    ...
    while (i < tokens.length) {
      if (result.length > 0) {
        ...
        const jr = joinWordsSyncFromRules(joinRules, prevSurface, tokens[i].surface);
        if (jr && jr.replaces_pair) {
...
function joinWordsSyncFromRules(joinRules, prev, nextWord) {
  const prevLower = prev.toLowerCase();
  const nextLower = nextWord.toLowerCase();
  for (const rule of joinRules) {
    if (!rule || typeof rule !== 'object') continue;
...
export async function createBuffer(locale, wordsData, rulesData) {
...
export async function findLayoutsByKeycode(keycode, layer = 'base') {
  const hid = normalizeKeycode(keycode);
  if (hid === null) return [];
  const matches = [];
  for (const id of AVAILABLE_LAYOUTS) {
    const kb = LAYOUTS[id];
    if (!kb) continue;
